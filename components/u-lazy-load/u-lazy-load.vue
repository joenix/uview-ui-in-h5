<template>
	<div class="u-wrap" :style="{
	 ...
	 :class="'u-lazy-item-' + elIndex">
		<div :class="'u-lazy-item-' + elIndex">
			<image :style="{borderRadius: borderRadius + 'px', height: imgHeight}" v-if="!isError" class="u-lazy-item"
			 :src="isShow ? image : loadingImg" :mode="imgMode" @load="imgLoaded" @error="loadError" @tap="clickImg"></image>
			<image :style="{borderRadius: borderRadius + 'px', height: imgHeight}" class="u-lazy-item error" v-else :src="errorImg"
...
<script>
	 ...
	 * @property {String Number} border-radius 图片圆角值，单位px（默认0）
