<template>
	<div class="u-subsection" :style="[subsectionStyle]">
		<div class="u-item u-line-1" :style="[itemStyle(index)]" @tap="click(index)" :class="[noBorderRight(index), 'u-item-' + index]"
		 v-for="(item, index) in listInfo" :key="index">
			<div :style="[textStyle(index)]" class="u-item-text u-line-1">{{ item.name }}</div>
		</div>
		<div class="u-item-bg" :style="[itemBarStyle]"></div>
	</div>
</template>
