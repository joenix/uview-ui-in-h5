	...
	<div class="">
		<movable-area class="u-swipe-action" :style="{ backgroundColor: bgColor }">
			...
			>
				...
				<div class="u-swipe-del" v-if="showBtn" @tap.stop="btnClick(index)" :style="[btnStyle(item.style)]" v-for="(item, index) in options" :key="index">
					<div class="u-btn-text">{{ item.text }}</div>
				</div>
			</movable-view>
		</movable-area>
	</div>
 ...
 * @property {String Number} btn-width 按钮宽度，单位px（默认180）
