<template>
	<div class="u-divider" :style="{
		height: height == 'auto' ? 'auto' : height + 'px',
		backgroundColor: bgColor,
		marginBottom: marginBottom + 'px',
		marginTop: marginTop + 'px'
	}" @tap="click">
		<div class="u-divider-line" :class="[type ? 'u-divider-line--bordercolor--' + type : '']" :style="[lineStyle]"></div>
		<div v-if="useSlot" class="u-divider-text" :style="{
			color: color,
 ...
 * @property {String Number} half-width 文字左或右边线条宽度，数值或百分比，数值时单位为px
 * @property {String} border-color 线条颜色，优先级高于type（默认#dcdfe6）
 * @property {String} color 文字颜色（默认#909399）
 * @property {String Number} fontSize 字体大小，单位px（默认26）
 * @property {String} bg-color 整个divider的背景颜色（默认呢#ffffff）
 * @property {String Number} height 整个divider的高度，单位px（默认40）
...
export default {
	...
	computed: {
		lineStyle() {
			...
			else style.width = this.halfWidth + 'px';
