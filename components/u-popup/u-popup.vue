<template>
	<div v-if="visibleSync" :style="[customStyle, {
		zIndex: uZindex - 1
		...
		<div
			...
			:style="[style]"
		>
			<div class="u-mode-center-box" @tap.stop.prevent @touchmove.stop.prevent v-if="mode == 'center'" :style="[centerStyle]">
				<u-icon
					@click="close"
					v-if="closeable"
					class="u-close"
			...
			<div @tap="close" class="u-close" :class="['u-close--' + closeIconPos]">
				<u-icon
...
export default {
	...
	props: {
		...
		// 抽屉的宽度(mode=left|right)，或者高度(mode=top|bottom)，单位px，或者"auto"
    ...
    methods: {
		...
		getUnitValue(val) {
			if(/(%|px|px|auto)$/.test(val)) return val;
			else return val + 'px'
		},
		// 遮罩被点击
		maskClick() {
