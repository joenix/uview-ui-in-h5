<template>
  <div v-if="show" class="u-tabbar" @touchmove.stop.prevent="() => {}">
    <div
    ...
    >
      ...
      >
        <div :class="[midButton && item.midButton ? 'u-tabbar__content__circle__button' : 'u-tabbar__content__item__button']">
          <u-icon
        ...
        >
          <span class="u-line-1">{{ item.text }}</span>
        </div>
      </div>
      <div
        v-if="midButton"
    ...
    <!-- 这里加上一个48px的高度,是为了增高有凸起按钮时的防塌陷高度(也即按钮凸出来部分的高度) -->
    <div
      class="u-fixed-placeholder safe-area-inset-bottom"
      :style="{
        height: `calc(${$u.addUnit(height)} + ${midButton ? 48 : 0}px)`
      }"
    ></div>
  </div>
</template>
