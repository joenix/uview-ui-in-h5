<template>
  <div class="u-avatar" :style="[wrapStyle]" @tap="click">
    <image @error="loadError" :style="[imgStyle]" class="u-avatar__img" v-if="!uText && avatar" :src="avatar" :mode="imgMode"></image>
    <span
      class="u-line-1"
      v-else-if="uText"
      :style="{
    ...
    >
    <slot v-else></slot>
    <div class="u-avatar__sex" v-if="showSex" :class="['u-avatar__sex--' + sexIcon]" :style="[uSexStyle]">
      <u-icon :name="sexIcon" size="20"></u-icon>
    </div>
    <div class="u-avatar__level" v-if="showLevel" :style="[uLevelStyle]">
 ...
 * @property {String Number} size 头像尺寸，可以为指定字符串(large, default, mini)，或者数值，单位px（默认default）
...
export default {
  ...
  props: {
    ...
    // 尺寸，large-大，default-中等，mini-小，如果为数值，则单位为px
    // 宽度等于高度
    size: {
      type: [String, Number],
      default: "default"
  ...
  computed: {
    wrapStyle() {
      let style = {};
      style.height = this.size == "large" ? "120px" : this.size == "default" ? "90px" : this.size == "mini" ? "70px" : this.size + "px";
      style.width = style.height;
      style.flex = `0 0 ${style.height}`;
      style.backgroundColor = this.bgColor;
      style.borderRadius = this.mode == "circle" ? "500px" : "5px";
      if (this.text) style.padding = `0 6px`;
      return style;
    },
    imgStyle() {
      let style = {};
