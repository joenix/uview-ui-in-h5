<template>
	<div class="u-upload" v-if="!disabled">
		<div
			v-if="showUploadList"
		...
		>
			...
			></u-line-progress>
			<div @tap.stop="retry(index)" v-if="item.error" class="u-error-btn">点击重试</div>
		...
		<div style="display: inline-block;" @tap="selectFile" v-if="maxCount > lists.length">
			...
			>
				...
				<div class="u-add-tips">{{ uploadText }}</div>
			</div>
		</div>
	</div>
</template>
