	...
	<div>
		<u-popup :zoom="zoom" mode="center" :popup="false" :z-index="uZIndex" v-model="value" :length="width"
		 :mask-close-able="maskCloseAble" :border-radius="borderRadius" @close="popupClose" :negative-top="negativeTop">
			<div class="u-model">
				<div v-if="showTitle" class="u-model__title u-line-1" :style="[titleStyle]">{{ title }}</div>
				<div class="u-model__content">
					<div :style="[contentStyle]" v-if="$slots.default  || $slots.$default">
						<slot />
					</div>
					<div v-else class="u-model__content__message" :style="[contentStyle]">{{ content }}</div>
				</div>
				<div class="u-model__footer u-border-top" v-if="showCancelButton || showConfirmButton">
					<div v-if="showCancelButton" :hover-stay-time="100" hover-class="u-model__btn--hover" class="u-model__footer__button"
					 :style="[cancelBtnStyle]" @tap="cancel">
						{{cancelText}}
					</div>
					<div v-if="showConfirmButton || $slots['confirm-button']" :hover-stay-time="100" :hover-class="asyncClose ? 'none' : 'u-model__btn--hover'"
...
<script>
	 ...
	 * @property {String | Number} border-radius 模态框圆角值，单位px（默认16）
