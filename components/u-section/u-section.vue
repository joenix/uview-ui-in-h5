<template>
	<div class="u-section">
		<div class="u-section__title" :style="{
			...
			<div class="u-section__title__icon-wrap u-flex" :style="[lineStyle]" v-if="showLine">
				<u-icon top="0" name="column-line" :size="fontSize * 1.25" bold :color="lineColor ? lineColor : color"></u-icon>
			</div>
			<span class="u-flex u-section__title__text">{{title}}</span>
		</div>
		<div class="u-section__right-info" v-if="right || $slots.right" :style="{
			color: subColor
		}" @tap="rightClick"> 
			<slot name="right" v-if="$slots.right" />
			<block v-else>
				{{subTitle}}
				<div class="u-section__right-info__icon-arrow u-flex" v-if="arrow">
		...
		computed: {
			...
			lineStyle() {
				...
				return {
					...
					top: -(Number(this.fontSize) * (this.$u.os() == 'ios' ? 0.14 : 0.15)) + 'px',
				}
			}
		},
		methods: {
