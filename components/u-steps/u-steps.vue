	...
	<div class="">
		...
		>
			...
			>
				...
				<div class="u-steps__item__dot" v-if="mode == 'dot'" :style="{ 
					backgroundColor: index <= current ? activeColor : unActiveColor 
				}"></div>
				<span class="u-line-1" :style="{ 
					color: index <= current ? activeColor : unActiveColor,
				}" :class="['u-steps__item__text--' + direction]">
					{{ item.name }}
				</span>
				<div class="u-steps__item__line" :class="['u-steps__item__line--' + mode]" v-if="index < list.length - 1">
...
$u-steps-item-number-width: 44px;
$u-steps-item-dot-width: 20px;
