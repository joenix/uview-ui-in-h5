<template>
	<div class="u-field" :class="{'u-border-top': borderTop, 'u-border-bottom': borderBottom }">
		<div class="u-field-inner" :class="[type == 'textarea' ? 'u-textarea-inner' : '', 'u-label-postion-' + labelPosition]">
			<div class="u-label" :class="[required ? 'u-required' : '']" :style="{
				justifyContent: justifyContent, 
				flex: labelPosition == 'left' ? `0 0 ${labelWidth}px` : '1'
			}">
				<div class="u-icon-wrap" v-if="icon">
					<u-icon size="32" :custom-style="iconStyle" :name="icon" :color="iconColor" class="u-icon"></u-icon>
				</div>
				<slot name="icon"></slot>
				<span class="u-label-text" :class="[this.$slots.icon || icon ? 'u-label-left-gap' : '']">{{ label }}</span>
			</div>
			<div class="fild-body">
				<div class="u-flex-1 u-flex" :style="[inputWrapStyle]">
					<spanarea v-if="type == 'textarea'" class="u-flex-1 u-textarea-class" :style="[fieldStyle]" :value="value"
 ...
 * @property {Number String} label-width label的宽度，单位px（默认130）
 * @property {String} label-align label的文字对齐方式（默认left）
 * @property {Object} field-style 自定义输入框的样式，对象形式
 * @property {Number | String} clear-size 清除图标的大小，单位px（默认30）
